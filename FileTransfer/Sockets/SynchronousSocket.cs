...
namespace FileTransfer.Sockets
{
    class SynchronousSocket
    {
        ...
        public void StartListening(IPAddress hostIP, int port)
        {
            try
            {
                LogHelper.Instance.Logger.Info(string.Format("开启本地端口{0}的侦听", port));
            ...
            catch (SocketException se)
            {
                ...
                if (se.ErrorCode == 10004)
                    return;
                LogHelper.Instance.Logger.Fatal(string.Format("启动本地端口{0}侦听发生套接字异常！SocketException ErrorCode:{1}", port, se.ErrorCode));
            ...
            {
                LogHelper.Instance.Logger.Error(string.Format("启动本地侦听时发生异常！异常：{0}", e.Message));
                CloseSocket(_listenSocket);
        ...
        public void StopListening()
        {
            ...
            {
                LogHelper.Instance.Logger.Fatal(string.Format("关闭本地监听发生套接字异常！SocketException ErrorCode:{0}", se.ErrorCode));
                MessageBox.Show("关闭本地侦听时发生套接字异常！", "错误", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            catch (Exception e)
            {
                LogHelper.Instance.Logger.Error(string.Format("关闭本地监听发生异常！异常：{0}", e.Message));
        ...
        public Socket StartConnecting(IPEndPoint ep)
        {
            ...
            {
                LogHelper.Instance.Logger.Error(string.Format("向远端{0}发起连接时发生套接字异常！SocketException ErrorCode:{1}", ep, se.ErrorCode));
                return null;
            }
            catch (Exception e)
            {
                LogHelper.Instance.Logger.Error(string.Format("向远端{0}发起连接时发生异常！异常：{1}", ep, e.Message));
                return null;
            }
        }
        ...
        private void CloseSocket(Socket socket)
        {
            ...
            _listenSocket.Close();
            _listenSocket = null;
            LogHelper.Instance.Logger.Info(string.Format("关闭本地侦听Socket并释放所有关联资源"));
        }
        #endregion
