...
namespace FileTransfer.Sockets
{
    public class SendFiles : SendProcess
    {
        ...
        protected override object SendParams(IPEndPoint remote, object[] param)
        {
            ...
            if (param == null || param.Length != 2) return sendedFiles;
            var monitorDirectory = (string)param[0];
            var monitorIncrement = param[1] as List<string>;
            ...
            try
            {
                ...
                {
                    //发送初始进度
                    SimpleIoc.Default.GetInstance<MainViewModel>().ShowSendProgress(monitorDirectory, remote.ToString(), file, 0.0);
                    //发送文件大小
                    sendBytes = new byte[8];
                    ...
                    using (FileStream fs = new FileStream(file, FileMode.Open, FileAccess.Read, FileShare.Read))
                    {
                        ...
                        while (index < fileSize)
                        {
                            ...
                            SimpleIoc.Default.GetInstance<MainViewModel>().ShowSendProgress(monitorDirectory, remote.ToString(), file, progress);
                            //设置文件流的当前位置
                    ...
                    //发送进度事件
                    SimpleIoc.Default.GetInstance<MainViewModel>().ShowSendProgress(monitorDirectory, remote.ToString(), file, 1.0);
                    //日志记录
            ...
            {
                string logMsg = string.Format("发送{0}下的文件至远端{1}的过程中发生异常！异常：{2}", monitorDirectory, remote, e.Message);
